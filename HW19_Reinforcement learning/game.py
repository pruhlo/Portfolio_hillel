...
class Runner():
  ...
  def run(self):
    ...
    # Check if ffmpeg is available and use it, otherwise default to Pillow
    try:
        ani.save('%s-movie.avi' % self.logs, writer='ffmpeg', dpi=300)
    except Exception as e:
        print(f"Warning: {e}")
        print("Falling back to saving as GIF.")
        ani.save('%s-movie.gif' % self.logs, writer='pillow', fps=2)
